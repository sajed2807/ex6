...
void playGame(GameState* g) {
    if(!g->player || g->roomCount==0) { printf("Init player and rooms first.\n"); return; }
    ...
    while(1) {
        ...
        if(c==1) {
            ...
            if(findRoomByXY(nx, ny, g->roomCount)) { x=nx; y=ny; }
            ...
            while(r->monster->hp>0 && g->player->hp>0) {
                ...
                if(r->monster->hp>0) { g->player->hp -= r->monster->attack; printf("Monster deals %d damage. Your HP: %d\n", r->monster->attack, g->player->hp); }
            }
            ...
            int o = getInt("Choice: ");
            if(o==1) bstPreorder(t->root, t->print);
            else if(o==2) bstInorder(t->root, t->print);
...
void freeGame(GameState* g) {
    ...
    while(r) {
        Room* next = r->next;
        if(r->monster) { free(r->monster->name); free(r->monster); }
        if(r->item) { free(r->item->name); free(r->item); }
        free(r);
        r = next;
    }
    if(g->player) {
        bstFree(g->player->bag->root, g->player->bag->freeData); free(g->player->bag);
        bstFree(g->player->defeatedMonsters->root, g->player->defeatedMonsters->freeData); free(g->player->defeatedMonsters);
        free(g->player);
    }
    free(g);
}
